...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--out",default="proposals/synonym_expansion.yaml"); args=parser.parse_args()
    ...
    for entry in entries:
        ...
        if aliases:
            proposals.append({"expression":entry["expression"],"current_aliases":entry.get("aliases",[]),"proposed_aliases":sorted(aliases),"requires_review":True})
    out=ROOT/args.out; out.parent.mkdir(parents=True,exist_ok=True)
    out.write_text(yaml.safe_dump({"version":"1.0.0","proposals":proposals},allow_unicode=True,sort_keys=False),encoding="utf-8")
