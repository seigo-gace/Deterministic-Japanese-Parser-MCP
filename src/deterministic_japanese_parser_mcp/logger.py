...
_stats = {"enqueued": 0, "sent": 0, "batches": 0, "failed": 0, "dropped": 0}
...
def _build_tgs_entry(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "message": json.dumps(safe_payload, ensure_ascii=False, separators=(",", ":"), sort_keys=True),
        "hint": "djpmcp-runtime",
...
def _post_tgs_bulk(entries: list[dict[str, Any]]) -> None:
    ...
    body = json.dumps({"logs": entries}, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    ...
    with urllib_request.urlopen(req, timeout=_timeout_seconds()) as response:
        ...
        if any(item.get("status") == "rejected" for item in results):
...
def _ensure_worker() -> queue.Queue[dict[str, Any]]:
    ...
    with _worker_lock:
        ...
        if _worker is None or not _worker.is_alive():
            _stop_event.clear()
            _worker = threading.Thread(target=_worker_loop, name="djpmcp-tgserver-log-sink", daemon=True)
            _worker.start()
    return _queue
...
def append_log(path: Path, payload: dict[str, Any]) -> None:
    """Emit diagnostics without putting log I/O on the parser request path."""
    mode = _sink_mode()
    if mode == "none":
        return
    if mode == "tgserver":
