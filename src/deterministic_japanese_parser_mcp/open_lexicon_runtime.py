...
_KANJI = re.compile('[一-龥々〆ヵヶ]')
_SQLITE_BACKEND = 'sqlite-index-v1'
_SQLITE_BACKEND_V2 = 'sqlite-index-v2'
...
class OpenLexiconRuntime:
    ...
    def __init__(self, root: Path, *, shard_cache_size: int=4):
        ...
        required_flags = {'exact_lookup_only': True, 'reading_alias_promotion': False, 'semantic_auto_promotion': False, 'intent_auto_promotion': False, 'external_action_auto_promotion': False}
        ...
        required_paths = {'surface': index_root / 'surface-index.json.gz', 'reading': index_root / 'reading-index.json.gz', 'locator': index_root / 'record-locator.json.gz'}
        missing = [str(path) for path in required_paths.values() if not path.exists()]
        if missing:
            raise FileNotFoundError('compiled open lexicon indexes are incomplete: ' + ', '.join(missing))
    ...
    def _open_sqlite_backend(self) -> None:
        ...
        if missing:
            connection.close()
            raise ValueError('compiled open lexicon sqlite tables are incomplete: ' + ', '.join(missing))
        count = int(connection.execute('SELECT COUNT(*) FROM records').fetchone()[0])
        if count != self.record_count:
            connection.close()
            raise ValueError(f'compiled open lexicon sqlite record count mismatch: manifest={self.record_count} sqlite={count}')
    ...
    def lookup_backend(self) -> str:
        if self._sqlite is not None:
            return self._sqlite_backend or _SQLITE_BACKEND
    ...
    def preload_records(self) -> None:
        ...
        if loaded_records != self.record_count:
            raise ValueError(f'compiled open lexicon preload count mismatch: expected={self.record_count} actual={loaded_records}')
        self.records_preloaded = True
    ...
    def _record(self, record_id: str) -> dict[str, Any]:
        if self._sqlite is not None:
            if self._sqlite_backend == _SQLITE_BACKEND_V2:
                row = self._sqlite.execute('\n                    SELECT record_id,lemma,reading,pos_json,domains_json,\n                           source_dataset,source_version,source_license\n                    FROM records WHERE record_id = ?\n                    ', (record_id,)).fetchone()
                if row is None:
    ...
    def _sqlite_surface_lookup(self, text: str, *, match_type: str, max_candidates: int) -> tuple[list[LexicalCandidate], int]:
        ...
        if self._sqlite_backend == _SQLITE_BACKEND_V2:
            rows = self._sqlite.execute('\n                SELECT r.record_id,r.lemma,r.reading,r.pos_json,r.domains_json,\n                       r.source_dataset,r.source_version,r.source_license,\n                       COUNT(*) OVER () AS total\n                FROM surface_lookup AS s\n                JOIN records AS r ON r.seq = s.record_seq\n                WHERE s.surface = ?\n                ORDER BY r.record_id\n                LIMIT ?\n                ', (text, max(1, max_candidates))).fetchall()
    ...
    def exact_lookup(self, text: str, *, match_type: str='surface', max_candidates: int=8) -> tuple[list[LexicalCandidate], int]:
        if not self.available or not text:
            return ([], 0)
        if self._sqlite is not None:
            return self._sqlite_surface_lookup(text, match_type=match_type, max_candidates=max_candidates)
        record_ids = self.surface_index.get(text, [])
        total = len(record_ids)
        selected = record_ids[:max(1, max_candidates)]
        return ([self._candidate(self._record(record_id), matched_text=text, match_type=match_type) for record_id in selected], total)

    def _sqlite_reading_lookup(self, reading: str, *, surface: str | None, normalized: str | None, max_candidates: int) -> tuple[list[LexicalCandidate], int]:
        assert self._sqlite is not None
        if self._sqlite_backend == _SQLITE_BACKEND_V2:
            ...
            for lookup_reading in lookup_values:
                rows = self._sqlite.execute('\n                    SELECT r.record_id,r.lemma,r.reading,r.pos_json,r.domains_json,\n                           r.source_dataset,r.source_version,r.source_license,\n                           COUNT(*) OVER () AS total\n                    FROM reading_lookup AS l\n                    JOIN records AS r ON r.seq = l.record_seq\n                    WHERE l.reading = ?\n                    ORDER BY r.record_id\n                    LIMIT ?\n                    ', (lookup_reading, max(1, max_candidates))).fetchall()
                if not rows:
                    continue
                total = int(rows[0][8])
                candidates = []
                for rid, lemma, stored_reading, pos_json, domains_json, dataset, version, license_name, _ in rows:
                    candidates.append(LexicalCandidate(record_id=rid, lemma=lemma, matched_text=lookup_reading, match_type='reading', readings=[stored_reading] if stored_reading else [], restricted_to=[], no_kanji=False, part_of_speech=json.loads(pos_json), domains=json.loads(domains_json), usage_labels=[], source_dataset=dataset, source_version=version, source_license=license_name))
                return (candidates, total)
            return ([], 0)
    ...
    def reading_lookup(self, reading: str, *, surface: str | None=None, normalized: str | None=None, max_candidates: int=8) -> tuple[list[LexicalCandidate], int]:
        if not self.available or not reading:
            return ([], 0)
        if self._sqlite is not None:
            return self._sqlite_reading_lookup(reading, surface=surface, normalized=normalized, max_candidates=max_candidates)
        allowed_surfaces = {value for value in (surface, normalized) if value}
        mappings: list[dict[str, Any]] = []
        lookup_reading = reading if reading in self.reading_index else _katakana_to_hiragana(reading)
        ...
        for mapping in mappings[:max(1, max_candidates)]:
            candidates.append(self._candidate(self._record(mapping['record_id']), matched_text=lookup_reading, match_type='reading', restricted_to=list(mapping.get('restricted_to', [])), no_kanji=bool(mapping.get('no_kanji', False))))
        return (candidates, total)
    ...
    def lookup_token(self, token: Token, *, max_candidates: int=8) -> Token:
        ...
        if total == 1:
            ...
            status = 'AMBIGUOUS'
        return token.model_copy(update={'lexical_candidates': candidates, 'lexical_candidate_total': total, 'lexical_status': status})
